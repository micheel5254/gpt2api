...
function downloadAbnormalEmails(accounts: Account[]) {
  const emails = accounts
    .filter((account) => account.status === "异常")
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = `abnormal-accounts-${Date.now()}.txt`;
  link.click();
  URL.revokeObjectURL(url);
}
      ...
      <section className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button
            ...
            onClick={() => downloadAbnormalEmails(accounts)}
            disabled={accounts.filter((a) => a.status === "异常").length === 0}
          >
            <Download className="size-4" />
            导出异常账号
          </Button>
